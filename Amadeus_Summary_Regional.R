...
desc_stat_by_file <- function(nuts_code, cfile, country, stat_variables = c("CP", "RoC")) {
    ...
    Cleaned_dat_INDEX <- merge(Cleaned_dat_INDEX, Cleaned_dat_RD, c("IDNR", "Year"))
    rm(Cleaned_dat_RD)
    
    retained_columns <- c("Year", nuts_code, stat_variables)
    Cleaned_dat_INDEX <- subset(Cleaned_dat_INDEX, select = retained_columns)
    colnames(Cleaned_dat_INDEX) <- c("Year", "nuts_code", stat_variables)
