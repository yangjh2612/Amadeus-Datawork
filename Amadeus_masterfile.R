...
fun_data_clean <- function(dat, def_d, def_d_list){
    ...
    data_c <- data_c %>%
        ...
        mutate(TURN_conv = TURN*EXCHRATE) %>% # 
        mutate(TOAS_conv = TOAS*EXCHRATE) %>% # C_PW_ratio 
      
        mutate(TURN_conv_def = TURN_conv/deflator*100) %>% # 
        mutate(TOAS_conv_def = TOAS_conv/deflator*100) %>% # C_PW_ratio 
...
fun_read_by_country <- function(filename_list, country_name_list, filename_nuts_list){             
    ...
    #print(country_names[c(18,26,31,32,40)])
    #print(country_names[c(44)])
    ...
    Cleaned_dat_INDEX <- data.frame(
        IDNR = IDNR, Year = CLOSDATE_year, NUTS_1 = NUTS_1, NUTS_2 = NUTS_2, NUTS_3 = NUTS_3, NAICs_two_digit = NAICS_two_digit, CONSOL = CONSOL, COMPCAT = COMPCAT, LSTATUS = LSTATUS, QUOTED = QUOTED, Firm_Age = Firm_Age, EXCHRATE = EXCHRATE, Deflator = deflator
    )
    ...
    Cleaned_dat_Productivity <- data.frame(
        IDNR = IDNR, Year = CLOSDATE_year, LP = LP_imp_conv_def, CP = CP_imp, LP_change = LP_change, CP_change = CP_change, Zeta = Zeta, CP_lroc = CP_lroc, LP_lroc = LP_lroc
    )

    Cleaned_dat_cost_structure <- data.frame(
        IDNR = IDNR, Year = CLOSDATE_year, WS = WS, PS = PS, PW_ratio = PW_ratio, C_com = C_com, C_com_lroc = C_com_lroc, PW_ratio_lroc = PW_ratio_lroc
    )

    Cleaned_dat_firm_size <- data.frame(
        IDNR = IDNR, Year = CLOSDATE_year, SALE = as.numeric(TURN_conv_def), EMPL =  as.numeric(EMPL), TOAS = as.numeric(TOAS_conv_def), SALE_change = TURN_change, EMPL_change = EMPL_change, TOAS_change = TOAS_change, VA = as.numeric(VA_imp), EMPL_lroc = EMPL_lroc, TOAS_lroc = TOAS_lroc, TURN_lroc = TURN_lroc
    )

    Cleaned_dat_RD <- data.frame(
        RD = as.numeric(RD), RD_Sales = RD_sales, RD_va = RD_va
    )
    ...
    save(
        ...
        file=paste("panels_J!&", paste(unlist(country_name_list), collapse=""), ".Rda", sep="")
    ) # either panels_ or consolidated_panels
}
...
#country_names <- country_names[c(1:12)]
#filenames <- filenames[c(1:12)]
#filenames_nuts <- filenames_nuts[c(1:12)]
...
#print(country_names[c(5,25,34,38,43)])
#all_ind <- 1:44
#sep_ind <- c(5, 25, 34, 36, 38, 43) #In  5, 25, 34, 36, 38, 43 has no consolidated data
#all_but_sep <- all_ind[-c(sep_ind)]
all_ind <- 1:44
sep_ind <- match(c("Bosnia and Herzegovina", "Macedonia, FYR", "Romania", "Slovenia", "Ukraine"), country_names)
sep_ind_noNA <- sep_ind[!is.na(sep_ind)]
all_but_sep <- setdiff(all_ind, sep_ind_noNA)

print("Commence reading and cleaning data...")

for (i in all_but_sep ) {
    #tryCatch({
    fun_read_by_country(filenames[[i]], country_names[[i]], filenames_nuts[[i]])      
    #}, error=function(e){})
    # function saves directly, so no need to save the return value
