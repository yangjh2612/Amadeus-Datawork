...
fun_levy_par <- function(pdf_name, title, dat_list, x_value, x_value_name, x_lab, leg_pos) { # this function has 5 arguments. 1) the name of the pdf file, 2) the title of the figure, 3) the data file that is generated from the fun_fit_levy function in "Prod_Fitting_Levy.R" script, 4) variable number: 1:Size, 2: Industry, 5) variable index: either "size" or "ind"
  ...
  axis(side = 1, at = x_value, label = x_value_name, lwd = 0.3, cex.axis = 0.7)
  axis(side = 2, lwd = 0.3, cex.axis = 0.7)
...
mtext(paste("Parameters of Levy Distribution: LP by Industry"), side = 3, line = 1, outer = TRUE, cex = 1.)



############ 3. Productivity by ind chart ############

fun_levy_par_ind <- function(dat_list) {
  five_ind <- which(country_names %in% country_names_five) # index for the top five countries
  ...
  for (k in five_ind) {
    par_a[[k]] <- unlist(lapply(levy_par[[k]], function(x) x[1]))
    par_b[[k]] <- unlist(lapply(levy_par[[k]], function(x) x[2]))
    par_g[[k]] <- unlist(lapply(levy_par[[k]], function(x) x[3]))
    par_d[[k]] <- unlist(lapply(levy_par[[k]], function(x) x[4]))
  }
  ...
  x_value <- c(1:20)
  x_value_name <- ind_name_table$ind_names_alphabet
  x_value_name_full <- c("Agr", "Mine", "Manu", "Elec", "Water", "Cons", "Whole", "Trans", "Accom", "Info", "F&I", "Real", "Prof-S", "Admin", "Public", "Edu", "Heath", "Art", "O-Serv", "House")
  x_lab <- "Industry"


  # a function to generate a dataframe for country, industry, and the parameter value
  comp_par <- function(c_name, x_ind, x_val) {
    this_ind <- x_value_name_full [which(c(1:20) %in% x_ind)]
    ok <- data.frame(Country = rep(c_name, length(this_ind)), Ind = this_ind, Value = x_val)
    return(ok)
  }

  country_names[[15]] <- "UK" # change the name: United Kingdon -> "UK"

  gg_result <- list() # ggplot2 result
  for (par_ind in 1:1) { # the first plot with the legend (any idea how not to repeat it?)

    ok_frame <- c() # a long format for the five countries
    for (k in 1:5) {
      ok <- comp_par(country_names[five_ind[k]], x_ind = c_uni_list[[five_ind[k]]], x_val = par_list[[par_ind]][[five_ind[k]]])

      ok_frame <- rbind(ok_frame, ok)
    }

    ok_frame[ok_frame$Ind %in% c("Real") & ok_frame$Country %in% c("Germany"), ] <- NA # remove the real estate in Germany: a very low Soofi ID.

    ok_frame <- na.omit(ok_frame)

    ok_frame_w <- dcast(ok_frame, Ind ~ Country) # a wide format

    ## the following two lines are for reordering of the stacked bar plot: it rearragnes the bar plot by its magnitude.
    ok_frame_w$total <- rowSums(ok_frame_w [, -1], na.rm = T)
    ok_frame_w$Ind <- factor(ok_frame_w$Ind, levels = ok_frame_w$Ind[order(ok_frame_w$total)])

    ok_frame <- melt(ok_frame_w, id.var = c("Ind", "total")) # again covert it to a long format for ggplot2

    col_gp2 <- brewer.pal(5, "Dark2") # color

    ## ggplot2. Not a big fan but the stacked barplot is so convenient in ggplot2. I used the ggplot2 way way long time ago so let me know if there is a better way than this.

    gg_result[[par_ind]] <- ggplot(ok_frame, aes(x = Ind, y = value, fill = variable)) +
      geom_bar(stat = "identity") +
      scale_fill_manual(values = c(col_gp2)) +
      coord_flip() + # vertical barplot
      labs(x = expression("Sector"), y = par_name_list[[par_ind]], size = 10) +
      ...
      theme(
        ...
        legend.position = c(0.75, .42), # manually adjusted legend position
  ...
  for (par_ind in 2:4) {
    ...
    ok_frame[ok_frame$Ind %in% c("Real") & ok_frame$Country %in% c("Germany"), ] <- NA
    ok_frame <- na.omit(ok_frame)

    ok_frame_w <- dcast(ok_frame, Ind ~ Country)
    ok_frame_w$total <- rowSums(ok_frame_w [, -1], na.rm = T)


    # reorder

    col_gp2 <- brewer.pal(5, "Dark2")
    ok_frame_w$Ind <- factor(ok_frame_w$Ind, levels = ok_frame_w$Ind[order(ok_frame_w$total)])


    ok_frame <- melt(ok_frame_w, id.var = c("Ind", "total"))

    gg_result[[par_ind]] <- ggplot(ok_frame, aes(x = Ind, y = value, fill = variable)) +
      geom_bar(stat = "identity") +
      scale_fill_manual(values = c(col_gp2)) +
      coord_flip() +
      labs(x = expression("Sector"), y = par_name_list[[par_ind]]) +
      guides(fill = FALSE) +
      theme(axis.text = element_text(size = 11), axis.title = element_text(size = 14)) +
      theme(
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
gg_lp_ind <- fun_levy_par_ind(dat_list = LP_ind_Levy_list_boot)

pdf(paste("Figure_Levy_Para_LP_Ind_Chart.pdf", sep = ""), height = 4, width = 15)
par(mfrow = c(1, 1), mar = c(3, 2.5, 1, 1), mgp = c(1.5, .3, 0), tck = -.01, oma = c(0, 0, 4, 0))

grid.arrange(gg_lp_ind[[1]], gg_lp_ind[[2]], gg_lp_ind[[3]], gg_lp_ind[[4]], ncol = 4, top = textGrob("Parameters of Levy Distribution: LP by Industry", gp = gpar(fontsize = 18, font = 8)))

dev.off()

#
gg_lp_g_ind <- fun_levy_par_ind(dat_list = LP_g_ind_Levy_list_boot)

pdf(paste("Figure_Levy_Para_LP_g_Ind_Chart.pdf", sep = ""), height = 4, width = 15)
par(mfrow = c(1, 1), mar = c(3, 2.5, 1, 1), mgp = c(1.5, .3, 0), tck = -.01, oma = c(0, 0, 4, 0))

grid.arrange(gg_lp_g_ind[[1]], gg_lp_g_ind[[2]], gg_lp_g_ind[[3]], gg_lp_g_ind[[4]], ncol = 4, top = textGrob("Parameters of Levy Distribution: LP_change by Industry", gp = gpar(fontsize = 18, font = 8)))
