...
## Basically the same plot function but augumented with the fitter line from the Levy distribution
...
size_not_con <- function(x){
  l <- length(x)
  ok <- c()
  for(i in 1:l){
  ok[i] <- which(size_names%in%x[i])
  }
...
fun_levy_par_growth <- function(pdf_name, title, dat_list, x_value_name, x_value, y_value, x_lab){
...
c_uni_list <- list(); levy_par <- list()
for(k in 1:length(dat_list[[1]])){

  c_uni_list[[k]] <- dat_list[[3]][[k]] # the numeric value of the unique class
  levy_par[[k]] <- lapply( dat_list[[1]][[k]], function(x) x[5]) # the corresponding levy parameters
}
...
l <- 1
   plot(x_value, y_value, yaxt = "n", xaxt = "n", main = par_name_list[[l]],  cex.main = 1., xlab = x_lab, ylab = ylab_list[[l]], cex = 0, ylim = c(min(unlist(par_list[[l]])), max(unlist(par_list[[l]]))*(1.05))) # x_value is the numeric value of all classes
 ...
 points(c_uni_list[[k]], par_list[[l]][[k]],  col = colores_this[k], cex = 0.6, pch = 20, type = "o", lty = 2, lwd = 1) # c_uni_list is the numeric value of the unique classes in the subsample
  }
...
for(l in 2:4){
 ...
 points(c_uni_list[[k]], par_list[[l]][[k]],  col = colores_this[k], cex = 0.6, pch = 20, type = "o", lty = 2, lwd = 1)
  }
...
## ------------------------------------------------------------------------
# I separate the level variables from the growth variables to put the \gamma and \delta variables in log scale. I could include the conditional statement in the previous function to achieve this, I did not want to make the plotting function too complicated. The only different between the level and growth plotting function is that the last two parameters are in log scale in the level plotting function. 
...
for(l in 3:4){
 ...
 points(c_uni_list[[k]], log(par_list[[l]][[k]]),  col = colores_this[k], cex = 0.6, pch = 20, type = "o", lty = 2, lwd = 1)
  }
