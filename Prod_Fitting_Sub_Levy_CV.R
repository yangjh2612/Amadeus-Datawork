...
# Sub_fun_LM is the main function to estimate the 4 parameter Subbotin distribution (L-Moment with Subbotin)
...
## cross validation  
CV_fun <- function(n_fold, n_rep, uni_data){
  test_levy_list <- list(); test_sub_list <- list()
  test_levy_list_all <- list(); test_sub_list_all <- list()
  
  
  for(k in 1:n_rep){ # the number of iteration
    
    uni_data_s <- sample(uni_data)
    folds <- cut(seq(1, length(uni_data_s)), breaks = n_fold, labels=FALSE)
    
    for(i in 1:n_fold){ # the number of division (n-f)
      print(paste(k, "and", i))
      ...
      train_levy <- Levy_fun_QT(trainD)
      
      train_sub <- Sub_fun_LM(trainD)
      
      
      test_levy <- sum(dstable(testD, train_levy[1], train_levy[2], train_levy[3], train_levy[4], log = T)) # log-likelihood of levy
      test_sub <- sum(log(pdfaep4(testD, train_sub)))  # log-likelihood of Sub
      
      test_levy_list[[i]] <- test_levy
      test_sub_list[[i]] <- test_sub
    }
    
    test_levy_list_all[[k]] <- sum(unlist(test_levy_list)) # the sum of all n-fold likelihood
    test_sub_list_all[[k]] <- sum(unlist(test_sub_list))
    
  }
  test_list_all <- list(test_levy_list_all, test_sub_list_all)
  ...
  for (k in 1:length(dat)) {
    ...
    if (nrow(zz) == 0) {
      ...
      for (c in 1:length(c_uni_name)) {
        ...
        c_list[[c]] <- CV_fun(n_fold = 10, n_rep = 10, uni_data = c_lp) # Cross validation function
